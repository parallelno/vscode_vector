...
{
  const activeHardware = getActiveHardware();
  const toolbarIsRunning = !isEmulatorPaused();
  ...
  if (isEmulatorPaused())
  {
    ...
    const dataHover = resolveDataDirectiveHoverForMemory(
      document,
      position,
      activeHardware,
      toolbarIsRunning,
  ...
  const symbol = resolveHoverSymbol(
    identifier, { filePath, line: position.line + 1 }, getSymbolCache());
  if (!symbol ) return undefined;

  if (symbol.kind === 'line')
  {
    const instructionHover = resolveInstructionHoverForMemory(
      activeHardware,
      document,
      position,
      symbol.value,
